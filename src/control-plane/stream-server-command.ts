...
interface ExecuteCommandContext {
  ...
  toPublicSessionController(controller: SessionControllerState | null): StreamSessionController | null;
...
if (command.type === 'conversation.update') {
  const updated = ctx.stateStore.updateConversationTitle(
...
if (command.type === 'stream.subscribe') {
  ...
  for (const entry of ctx.streamJournal) {
    ...
    const diagnosticSessionId = ctx.diagnosticSessionIdForObservedEvent(
      entry.scope,
      entry.event,
    );
