...
void test('task screen keybindings default and override resolution are stable', () => {
  ...
  assert.equal(firstTaskScreenShortcutText(overridden, 'mux.home.task.submit'), 'ctrl+enter');
  assert.equal(firstTaskScreenShortcutText(overridden, 'mux.home.editor.cursor.left'), 'alt+h');
...
void test('task screen keybinding parser ignores malformed sequences and malformed binding tokens', () => {
  ...
  assert.equal(detectTaskScreenKeybindingAction(Buffer.from('plain-text', 'utf8'), bindings), null);
  assert.equal(
  ...
  assert.equal(detectTaskScreenKeybindingAction(Buffer.from('\u001b[2~', 'utf8'), bindings), null);
  assert.equal(
    detectTaskScreenKeybindingAction(Buffer.from('\u001b[xyzu', 'utf8'), bindings),
    null,
  );
  assert.equal(detectTaskScreenKeybindingAction(Buffer.from([0x1b, 0x80]), bindings), null);
  assert.equal(detectTaskScreenKeybindingAction(Buffer.from([0x80]), bindings), null);
});
...
void test('task screen keybinding coverage matrix exercises protocol decode branches', () => {
  ...
  assert.equal(
    detectTaskScreenKeybindingAction(Buffer.from([0x1b]), bindings),
    'mux.home.editor.line.end',
  );
  assert.equal(
    detectTaskScreenKeybindingAction(Buffer.from([0x20]), bindings),
    'mux.home.task.submit',
  );
  assert.equal(
    detectTaskScreenKeybindingAction(Buffer.from([0x7f]), bindings),
  ...
  assert.equal(detectTaskScreenKeybindingAction(Buffer.from('\u001b[4~', 'utf8'), bindings), null);
  assert.equal(
