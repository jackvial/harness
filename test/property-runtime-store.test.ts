...
function distortToken(token: string, casing: readonly boolean[], separators: readonly string[]): string {
...
void test('property: claude notification token normalization is punctuation/case invariant', () => {
  ...
  fc.assert(
    fc.property(
      fc.constantFrom('permissionapproved', 'permissiongranted', 'approvalapproved', 'approvalgranted'),
    ...
    fc.property(
      fc.array(
        fc.record({
          ...
          eventShape: fc.constantFrom<'provider-turn' | 'meta-queue'>('provider-turn', 'meta-queue'),
        }),
      ...
      (seedEvents, pivotSeed) => {
        ...
        try {
          const events: NormalizedEventEnvelope[] = seedEvents.map((seed, index) => ({
            ...
            type: seed.eventShape === 'provider-turn' ? 'provider-turn-started' : 'meta-queue-updated',
