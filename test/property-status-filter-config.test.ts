...
import { eventIncludesRepositoryId, eventIncludesTaskId, matchesObservedFilter } from '../src/control-plane/stream-server-observed-filter.ts';
import type { StreamObservedEvent, StreamSessionStatusModel } from '../src/control-plane/stream-protocol.ts';
...
void test('property: codex reducer ignores invalid telemetry observedAt for work-state updates', () => {
  fc.assert(
    fc.property(timestampArb, timestampArb, invalidTimestampArb, (previousAt, observedAt, invalidTs) => {
