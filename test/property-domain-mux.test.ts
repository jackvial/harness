  ...
  fc.assert(
    fc.property(
      ...
      (seedTasks, fromSeed, toSeed) => {
        ...
        const sortByOrder = (list: readonly TestTask[]) => [...list].sort((a, b) => a.order - b.order);
  ...
  fc.assert(
    fc.property(
      ...
      (seedTasks) => {
        ...
        const activeIds = ordered.filter((task) => task.status !== 'completed').map((task) => task.taskId);
        const completedIds = ordered.filter((task) => task.status === 'completed').map((task) => task.taskId);
  ...
  fc.assert(
    fc.property(
      ...
      fc.uniqueArray(
        fc.record({
          sessionId: idArb,
          directoryId: fc.option(fc.string({ minLength: 0, maxLength: 8 }), {
            nil: null,
          }),
      ...
      (directoryIds, conversations, extraOrderedIds) => {
        ...
        const orderedSessionIds = [
          ...new Set([...extraOrderedIds, ...conversations.map((conversation) => conversation.sessionId)]),
        ];
        ...
        const leftNav = buildLeftNavSelectorEntries(directories, conversationMap, orderedSessionIds, {
          includeHome: true,
        });
...
void test('property: shortcut decoder accepts equivalent ctrl encodings', () => {
  ...
  fc.assert(
    fc.property(fc.constantFrom(...safeCtrlLetters), (letter) => {
      ...
      assert.equal(
        detectMuxGlobalShortcut(Buffer.from(`\u001b[${String(letter.charCodeAt(0))};5u`, 'utf8'), bindings),
