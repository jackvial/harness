...
void test('selector index follows directory order then per-directory session order', () => {
  ...
  >([
    [
      'session-1',
      { sessionId: 'session-1', directoryId: 'dir-b', title: 'b-1', agentType: 'codex' },
    ],
    [
      'session-2',
      { sessionId: 'session-2', directoryId: 'dir-a', title: 'a-1', agentType: 'codex' },
    ],
    [
      'session-3',
      { sessionId: 'session-3', directoryId: 'dir-a', title: 'a-2', agentType: 'terminal' },
    ],
  ]);
...
void test('selector index appends unknown and null directories after known directories', () => {
  ...
  >([
    [
      'session-known',
      { sessionId: 'session-known', directoryId: 'dir-a', title: 'known', agentType: 'codex' },
    ],
    [
      'session-unknown',
      { sessionId: 'session-unknown', directoryId: 'dir-z', title: 'unknown', agentType: 'codex' },
    ],
    [
      'session-null',
      { sessionId: 'session-null', directoryId: null, title: 'null', agentType: 'terminal' },
    ],
  ]);
...
void test('selector index ignores unknown ordered session ids and normalizes blank directory ids', () => {
  ...
  >([
    [
      'session-1',
      { sessionId: 'session-1', directoryId: 'dir-a', title: 'known', agentType: 'codex' },
    ],
    [
      'session-2',
      { sessionId: 'session-2', directoryId: '   ', title: 'blank', agentType: 'terminal' },
    ],
  ]);
...
void test('visual conversation order matches rendered project grouping for thread cycling', () => {
  ...
  >([
    [
      'session-b-1',
      { sessionId: 'session-b-1', directoryId: 'dir-b', title: 'b-1', agentType: 'codex' },
    ],
    [
      'session-a-1',
      { sessionId: 'session-a-1', directoryId: 'dir-a', title: 'a-1', agentType: 'codex' },
    ],
    [
      'session-b-2',
      { sessionId: 'session-b-2', directoryId: 'dir-b', title: 'b-2', agentType: 'terminal' },
    ],
  ]);
...
void test('left nav selector order omits home when includeHome is false or omitted', () => {
  ...
  assert.deepEqual(
    omitted.map((entry) => entry.key),
    ['directory:dir-a', 'conversation:session-a'],
  );
  assert.deepEqual(
    explicitFalse.map((entry) => entry.key),
    ['directory:dir-a', 'conversation:session-a'],
  );
});
