...
void test('focused pane handles empty/archived repository states and dropdown selection rows', () => {
  ...
  assert.equal(view.rows.some((row) => row.includes('● live-repo')), true);
  assert.equal(view.rows.some((row) => row.includes('zzz')), false);
  assert.equal(view.rows.some((row) => row.includes('notice: pick one')), true);
  assert.equal(view.rows.some((row) => row.includes('no tasks yet')), true);
});
...
void test('focused pane supports row/cell hit testing and row clamping', () => {
  ...
  assert.equal(
    taskFocusedPaneActionAtCell(view, taskRowIndex, row.length + 50),
    'task.focus'
  );
...
void test('focused pane handles tight viewport and scroll clamping', () => {
  ...
  assert.equal(
    taskFocusedPaneRepositoryIdAtRow(view, -1),
    view.repositoryIds[0] ?? null
  );
});
...
void test('focused pane task ordering handles timestamp fallbacks and no-repository branch', () => {
  ...
  assert.equal(view.rows.some((row) => row.includes('no repository selected')), true);
});
...
void test('focused pane truncates narrow notice rows to ellipsis', () => {
  ...
  assert.equal(view.rows.some((row) => row.includes('notice: …')), true);
});
