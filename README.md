...
- Mux supports first-party in-pane selection mode (`Ctrl-\`), visual highlight, and clipboard copy (`y` or `Enter`).
...
- Mux paint correctness:
  ...
  - press `Ctrl-\` to switch to `select=custom`, drag with mouse in left pane to select, press `y` (or `Enter`) to copy, `Esc` to clear
