...
- `WorkspaceModel` now owns pane transition state mutations (`enterProjectPane`, `enterHomePane`) so project/home mode and related selection/edit-click reset behavior are modeled as domain state transitions instead of inline runtime field assignment blocks.
- `WorkspaceModel` now also owns interaction/UI ephemeral state used by mux runtime (modal prompt state, pointer-selection state, divider-drag state, title-edit state, latest rail rows), so runtime and input modules mutate one shared model instead of duplicating local `let` state bags.
...
- Control-plane parsing/wrapping is moving into a class-based service layer (`src/services/control-plane.ts`) so runtime callsites consume validated domain records instead of duplicating command parsing; task/repository/directory + conversation metadata, PTY/session lifecycle, startup session hydration (`session.list`/`session.status`), startup git-status hydration, and repository mutation flows now route through this service.
- Event batching/flush persistence is moving into a class-based `EventPersistence` service (`src/services/event-persistence.ts`) so runtime delegates timer-based event flush scheduling, batch-size immediate flush, perf span attribution, and error reporting.
- Mux UI state persistence is moving into a class-based `MuxUiStatePersistence` service (`src/services/mux-ui-state-persistence.ts`) so runtime delegates debounced config writes, unchanged-state suppression, and write failure reporting.
- Process-usage sampling is moving into a class-based `ProcessUsageRefreshService` (`src/services/process-usage-refresh.ts`) so runtime delegates in-flight guards, process usage map ownership, and background refresh span/change handling.
- Session projection/selector instrumentation is moving into a class-based `SessionProjectionInstrumentation` service (`src/services/session-projection-instrumentation.ts`) so runtime delegates selector snapshot hashing/indexing and transition telemetry emission.
- Output/render load sampling is moving into a class-based `OutputLoadSampler` service (`src/services/output-load-sampler.ts`) so runtime delegates output-chunk sampling, render/output duration aggregation, event-loop delay sampling, perf status-row state, and periodic `mux.output-load.sample` emission.
- Runtime teardown sequencing is moving into a class-based `RuntimeShutdownService` (`src/services/runtime-shutdown.ts`) so runtime delegates ordered shutdown (listener teardown, close/drain, persistence flush, recording/store/terminal close, startup/perf finalize) to one service seam.
- Process/signal listener orchestration is moving into a class-based `RuntimeProcessWiring` (`src/services/runtime-process-wiring.ts`) so runtime delegates attach/detach of stdin/resize/signal handlers and guarded fatal routing for `onInput`/`onResize`.
- Render/fatal lifecycle state is moving into a class-based `RuntimeRenderLifecycle` (`src/services/runtime-render-lifecycle.ts`) so runtime delegates dirty-mark scheduling, guarded render recursion, fatal runtime handling, and fatal-exit timer lifecycle.
- Conversation action-handler orchestration is moving into a class-based `RuntimeConversationActions` (`src/services/runtime-conversation-actions.ts`) so runtime delegates create/activate, critique-open-or-create, and takeover claim/apply flows instead of maintaining inline callback-heavy lifecycle wrappers.
- Conversation start lifecycle orchestration is moving into a class-based `RuntimeConversationStarter` (`src/services/runtime-conversation-starter.ts`) so runtime delegates launch-arg resolution, PTY start/resize wiring, startup-span updates, and session-summary subscription flow instead of keeping inline start logic.
- Stream envelope routing is moving into a class-based `RuntimeEnvelopeHandler` (`src/services/runtime-envelope-handler.ts`) so runtime delegates PTY output/event/exit handling, normalized event enqueueing, adapter-state merges, and observed event fanout instead of maintaining a large inline envelope branch tree.
- Render flush composition is moving into a class-based `RuntimeRenderFlush` (`src/services/runtime-render-flush.ts`) so runtime delegates status-footer composition, modal overlay application, screen flush delegation, and recording/startup flush side-effects instead of maintaining inline flush orchestration.
- Render state preparation is moving into a class-based `RuntimeRenderState` (`src/services/runtime-render-state.ts`) so runtime delegates pane-active gating, active-frame selection, and drag-selection payload assembly instead of keeping render-state branching inline.
- Render sequencing is moving into a class-based `RuntimeRenderOrchestrator` (`src/services/runtime-render-orchestrator.ts`) so runtime delegates dirty/shutdown gates and rail/right/flush orchestration ordering instead of carrying that composition in the main runtime body.
- Right-pane row assembly is moving into a class-based `RuntimeRightPaneRender` (`src/services/runtime-right-pane-render.ts`) so runtime delegates conversation/home/project row branching and workspace pane-scroll/selection snapshot updates instead of maintaining inline right-pane render branches.
- Left-rail render assembly is moving into a class-based `RuntimeLeftRailRender` (`src/services/runtime-left-rail-render.ts`) so runtime delegates selector instrumentation refresh plus left-rail render input wiring instead of composing rail input payloads inline.
- Directory/conversation archive lifecycle orchestration is moving into a class-based `RuntimeDirectoryActions` (`src/services/runtime-directory-actions.ts`) so runtime delegates archive conversation, add-directory hydration, and close-directory sequencing instead of maintaining inline callback-heavy directory/thread action wrappers.
- Conversation activation lifecycle orchestration is moving into a class-based `RuntimeConversationActivation` (`src/services/runtime-conversation-activation.ts`) so runtime delegates active-thread switching, attach retry behavior, and pane-transition side effects instead of maintaining inline activation control flow.
- Task-pane selection/editor transitions are moving into a class-based `TaskPaneSelectionActions` (`src/services/task-pane-selection-actions.ts`) so runtime delegates draft/task/repository focus-selection state changes plus task/repository reconcile flows (`syncTaskPaneSelection*`) instead of keeping task-pane transition handlers inline.
- Task-planning startup hydration is moving into a class-based `TaskPlanningHydrationService` (`src/services/task-planning-hydration.ts`) so runtime delegates repository/task list hydration sequencing and selection resync flow.
- Task-planning observed-event reducers are moving into a class-based `TaskPlanningObservedEvents` (`src/services/task-planning-observed-events.ts`) so runtime delegates repository/task observed-event branch handling instead of maintaining inline branch-heavy reducers.
- Directory startup hydration is moving into a class-based `DirectoryHydrationService` (`src/services/directory-hydration.ts`) so runtime delegates directory-list normalization/repair and active-directory resolution instead of maintaining inline startup hydration loops.
- Conversation startup hydration is moving into a class-based `ConversationStartupHydrationService` (`src/services/conversation-startup-hydration.ts`) so runtime delegates startup conversation/session list hydration, summary upsert, and live-session subscription sequencing.
- Startup state hydration orchestration is moving into a class-based `StartupStateHydrationService` (`src/services/startup-state-hydration.ts`) so runtime delegates startup hydration sequencing (conversation + repository + task-planning + git snapshot + initial left-nav selection) instead of maintaining inline startup orchestration blocks.
- Recording shutdown lifecycle is moving into a class-based `RecordingService` (`src/services/recording.ts`) so runtime delegates recording-writer close, gif export, and recording status/error emission behavior to one service seam.
- Startup span lifecycle tracking is moving into a class-based `StartupSpanTracker` (`src/services/startup-span-tracker.ts`) so runtime delegates startup span begin/end ownership (start-command, first-output, first-paint, settled) to one service seam.
- Startup render visibility helpers are moving into a class-based `StartupVisibility` (`src/services/startup-visibility.ts`) so runtime delegates startup glyph/header visibility calculations used by first-paint and settled gating.
- Startup settled-gate orchestration is moving into a class-based `StartupSettledGate` (`src/services/startup-settled-gate.ts`) so runtime delegates settled timer/probe perf wiring and settled-span completion flow.
- Startup background-probe orchestration is moving into a class-based `StartupBackgroundProbeService` (`src/services/startup-background-probe.ts`) so runtime delegates settled-wait/timeout gating and background interval lifecycle ownership.
- Startup background-resume orchestration is moving into a class-based `StartupBackgroundResumeService` (`src/services/startup-background-resume.ts`) so runtime delegates settled/timeout wait + background queue event lifecycle ownership.
- Background persisted-conversation queue orchestration is moving into a class-based `StartupPersistedConversationQueueService` (`src/services/startup-persisted-conversation-queue.ts`) so runtime delegates non-live conversation queue selection, recheck-before-start guards, and background start scheduling labels.
- Startup first-output startup-session tracking is moving into a class-based `StartupOutputTracker` (`src/services/startup-output-tracker.ts`) so runtime delegates per-session first-output dedupe and active-target first-output span completion.
- Startup first-paint/header/gate tracking is moving into a class-based `StartupPaintTracker` (`src/services/startup-paint-tracker.ts`) so runtime delegates active-target paint/header/gate perf wiring and settled-probe scheduling.
- Startup shutdown finalization is moving into a class-based `StartupShutdownService` (`src/services/startup-shutdown.ts`) so runtime delegates startup span completion and settled-gate teardown sequencing.
...
- Debug-footer notice TTL/trim/expiry behavior is moving behind a class-based `DebugFooterNotice` (`src/ui/debug-footer-notice.ts`) so runtime delegates transient footer-notice lifecycle rather than owning ad-hoc notice state/timing logic.
...
- Transactional append-only SQLite `events` persistence in `src/store/event-store.ts` (tenant/user scoped reads).
  ...
  - `scripts/codex-live-mux.ts` provides the first-party split UI (left: workspace rail, right: live steerable Codex session rendered via shared snapshot oracle) with:
    ...
    - Cursor yolo launch mode appends `--yolo` for interactive launches (while honoring explicit `--force`) and only appends `--trust` when launch args are print/headless mode (Cursor constraint), while still reusing persisted Cursor resume IDs for `--resume` session continuity.
  ...
  - Bun runtime debugger endpoints for long-running gateway/client processes are config-governed via `debug.inspect` (`enabled`, `gatewayPort`, `clientPort`) and wired into harness-managed Bun exec paths as deterministic inspector paths (`--inspect=localhost:<port>/harness-gateway` and `--inspect=localhost:<port>/harness-client`) for live profile attach.
