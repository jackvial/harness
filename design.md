...
- Left-rail status icon/detail rendering is intentionally suppressed when `statusModel` is `null` (notably `terminal` and `critique` threads), keeping those rows title-only.
